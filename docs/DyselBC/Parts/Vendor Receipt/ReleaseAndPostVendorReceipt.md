...
Once a Vendor Receipt reflects what physically arrived, you release it and post it to
bring the goods into stock. This guide also explains what happens when there is a
shortage or damage.
...
> You can skip this step and post directly — see Step 2 — in which case the system
> releases the document for you after a confirmation.
...
1. Receives each linked Purchase Order using the standard Business Central receipt
   process, **receive only** (it does not invoice).
...
> worksheet that drives standard posting — it does not invent its own inventory
> logic. That means costing, GL, and item ledger entries all behave exactly as they
> do for a normal purchase receipt.
...
1. The system creates a **Purchase Return Order** against the **Claim Vendor No.**
   on the header.
...
> Release will stop you if a claim quantity exists without a claim vendor.
...
sales order, or onward via a transfer, continue to the
**[Put-away List](PutAwayList.md)**.
