...
| 67410 | Parts | Fixed the issue causing the unit price not be calculated correct for customers with a currency code and using sales price calculation. |
...
| 67404 | Finance | Created a new report Adjust Cost - Item Entries ELC which does not stop in case there is an error message but it prints the line with the latest error message. If the report does not print any Item, it means that it ran succesful. With the Inventory Cost Adjustment it is also possible to run the Adjust Cost - Item Entries for all items or for a selection. |
...
If the shipment functionality is not enabled, there will be no change in process for how service types are changed on work order headers or lines.  |
...
| 67316 | Mobile Field Service | Added Estimated Inventory to the Mobile App.
Setup needed: daily Job Queue; Update SKU DMB ELC |
